...
impl<S: State> Client<S> {
    ...
    pub async fn tick_size(&self, token_id: &str) -> Result<TickSizeResponse> {
        if let Some(tick_size) = self.inner.tick_sizes.get(token_id) {
            trace!(token_id = %token_id, tick_size = ?tick_size.value(), "cache hit: tick_size");
            return Ok(TickSizeResponse {
                minimum_tick_size: *tick_size,
            });
        }

        trace!(token_id = %token_id, "cache miss: tick_size");
        ...
        trace!(token_id = %token_id, "cached tick_size");
    ...
    pub async fn neg_risk(&self, token_id: &str) -> Result<NegRiskResponse> {
        if let Some(neg_risk) = self.inner.neg_risk.get(token_id) {
            trace!(token_id = %token_id, neg_risk = *neg_risk, "cache hit: neg_risk");
            return Ok(NegRiskResponse {
                neg_risk: *neg_risk,
            });
        }

        trace!(token_id = %token_id, "cache miss: neg_risk");
        ...
        trace!(token_id = %token_id, "cached neg_risk");
    ...
    pub async fn fee_rate_bps(&self, token_id: &str) -> Result<FeeRateResponse> {
        if let Some(base_fee) = self.inner.fee_rate_bps.get(token_id) {
            trace!(token_id = %token_id, base_fee = *base_fee, "cache hit: fee_rate_bps");
            return Ok(FeeRateResponse {
                base_fee: *base_fee,
            });
        }

        trace!(token_id = %token_id, "cache miss: fee_rate_bps");
        ...
        trace!(token_id = %token_id, "cached fee_rate_bps");
